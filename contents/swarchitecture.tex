...
Il nodo Vision si occupa di analizzare le immagini provenienti dalla telecamera di Spykee, per rilevare la presenza di una torre o di una fabbrica all'interno dell'immagine. Riceve da \emph{SpyKee} i messaggi contenenti le immagini e, analizzata l'immagine, pubblica un messaggio di tipo \verb|Vision::Results| sull'argomento \verb|vision_results| contenente i dati riguardanti gli oggetti trovati e la loro posizione rispetto al centro dell'immagine. %TODO DETTAGLIARE MESSAGGI
...
Poiché l'analisi che viene effettuata per identificare le regioni di interesse si basa su un algoritmo di tipo KNN, è necessaria una prima fase di addestramento, che permette di ottenere un classificatore (un file \verb|.kcc|), mediante il quale è possibile associare ad ogni pixel dell'immagine, ossia ad ogni valore $(r,g,b)$, una classe (``torre'', ``fabbrica'' o ``niente'').
...
IsAac si occupa di controllare il comportamento di basso livello del robot durante il gioco. Questo nodo ignora le problematiche relative al conteggio dei punti, all'avvio e alla conclusione della partita, e gestisce semplicemente il comportamento del robot (i set-point per i cingoli e l'accensione dei led presenti sul robot) in base alle regole del gioco e ai dati provenienti dai sensori. 
 ...
 \emph{IsAac} riceve i messaggi pubblicati da Echoes e Vision, e invia comandi a SpyKee (messaggi \verb|Spykee::Motion| sull'argomento \verb|spykee_motion|) riguardanti il controllo dei cingoli. Inoltre invoca il servizio \verb|led_data| esposto da Echoes per il controllo dei led posti sul robot. Inoltre comunica con \verb|RoboTower_Game| per quanto concerne l'avvio e l'arresto del movimento del robot e la riattivazione dei tag RFID (che invece vengono disattivati da IsAac quando vengono rilevati).

%TODO espandere?
I comportamenti che sono stati implementati mediante regole fuzzy riguardano:
\begin{itemize}
...
%TODO bisogna specificare come viene calcolato il punteggio ;)
%TODO segnare funzionamento dei led...

%TODO mettere settaggi in un file anziché come costanti nel codice...
